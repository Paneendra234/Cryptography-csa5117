...
def permute(bits, table):
    """Permutes bits according to a mapping table (1-indexed)."""
    return [bits[i - 1] for i in table]


def left_shift(bits, n):
    """Circular left shift on a bit list."""
    return bits[n:] + bits[:n]


def generate_subkeys(key_64bit):
    """Generates 16 round subkeys (48 bits each) from a 64-bit key."""
    pc1_key = permute(key_64bit, PC1)
...
def des_process_block(block_64bit, subkeys):
    """Runs 16 rounds of DES on a single 64-bit block."""
    ip_block = permute(block_64bit, IP)
    l, r = ip_block[:32], ip_block[32:]

    for subkey in subkeys:
        f_res = feistel_f(r, subkey)
...
def hex_to_bits(hex_str):
    """Converts a hex string into a list of 0/1 bits."""
    val = int(hex_str, 16)
...
def bits_to_hex(bits):
    """Converts a list of 0/1 bits into a uppercase hex string."""
    val = 0
    for b in bits:
        val = (val << 1) | b
    num_hex_chars = len(bits) // 4
...
if __name__ == "__main__":
    ...
    cipher_bits = des_process_block(plaintext_bits, subkeys)
    cipher_hex = bits_to_hex(cipher_bits)

    decrypted_bits = des_process_block(cipher_bits, subkeys[::-1])
    decrypted_hex = bits_to_hex(decrypted_bits)
